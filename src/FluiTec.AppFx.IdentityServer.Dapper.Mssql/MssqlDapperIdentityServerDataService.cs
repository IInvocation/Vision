...
namespace FluiTec.AppFx.IdentityServer.Dapper.Mssql
{
	...
	public class MssqlDapperIdentityServerDataService : MssqlDapperDataService, IIdentityServerDataService
	{
		...
		{
			RegisterRepositoryProvider(new Func<IUnitOfWork, IApiResourceClaimRepository>(work => new MssqlApiResourceClaimRepository(work)));
			RegisterRepositoryProvider(new Func<IUnitOfWork, IApiResourceRepository>(work => new MssqlApiResourceRepository(work)));
			RegisterRepositoryProvider(new Func<IUnitOfWork, IApiResourceScopeRepository>(work => new MssqlApiResourceScopeRepository(work)));
			RegisterRepositoryProvider(new Func<IUnitOfWork, IClientRepository>(work => new MssqlClientRepository(work)));
			RegisterRepositoryProvider(new Func<IUnitOfWork, IClientScopeRepository>(work => new MssqlClientScopeRepository(work)));
			RegisterRepositoryProvider(new Func<IUnitOfWork, IIdentityResourceClaimRepository>(work => new MssqlIdentityResourceClaimRepository(work)));
			...
			RegisterRepositoryProvider(new Func<IUnitOfWork, ISigningCredentialRepository>(work => new MssqlSigningCredentialRepository(work)));
		}
